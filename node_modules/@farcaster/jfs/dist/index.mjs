// src/index.ts
import { isAddress, verifyMessage, isHex, hexToBytes as hexToBytes2 } from "viem";
...
// src/index.ts
function toJsonFarcasterSignature(data) {
...
function decode(input) {
  ...
  const payload = data.payload ? JSON.parse(Buffer.from(fromBase64Url(data.payload), "base64").toString("utf-8")) : void 0;
...
async function verify({
  ...
  if (decoded.header.type === "app_key") {
    const verifyResult = ed25519.verify(
      ...
      hexToBytes2(decoded.header.key)
    );
    if (!verifyResult) {
      throw new Error("Invalid signature");
    }
